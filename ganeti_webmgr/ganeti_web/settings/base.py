...
def create_secrets(folder='.secrets'):
    ...
    if not exists(secrets_folder):
        try:
            ...
            print ('Unable to create directory, at %s. Please make sure to '
                   'set the SECRET_KEY and WEB_MGR_API_KEY setting in '
                   'config.yml' % secrets_folder)
            return
    ...
    try:
        # File containing secretkey doesnt exist, so create it and fill it with
        # the key
        ...
        msg = ("Unable to %s file at %s. Please either create the file and "
               "ensure it contains a 32bit random value or ensure you have "
