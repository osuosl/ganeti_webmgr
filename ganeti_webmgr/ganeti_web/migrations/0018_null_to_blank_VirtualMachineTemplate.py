...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # nic_type, root_path, template_name, cdrom2_image_path,
        # cdrom_image_path, kernel_path, description,
        # boot_order, pnode, snode, disk_type, iallocator_hostname
        vmts = orm['ganeti_web.VirtualMachineTemplate'].objects
        vmts.filter(nic_type__isnull=True).update(nic_type='')
        vmts.filter(root_path__isnull=True).update(root_path='')
        vmts.filter(template_name__isnull=True).update(template_name='')
        vmts.filter(cdrom2_image_path__isnull=True).update(cdrom2_image_path='')
        vmts.filter(cdrom_image_path__isnull=True).update(cdrom_image_path='')
        vmts.filter(kernel_path__isnull=True).update(kernel_path='')
        vmts.filter(description__isnull=True).update(description='')
        vmts.filter(boot_order__isnull=True).update(boot_order='')
        vmts.filter(pnode__isnull=True).update(pnode='')
        vmts.filter(snode__isnull=True).update(snode='')
        vmts.filter(disk_type__isnull=True).update(disk_type='')
        vmts.filter(iallocator_hostname__isnull=True).update(iallocator_hostname='')

    def backwards(self, orm):
        raise RuntimeError("This migration is not backwards compatible")
